...
from ..utils.Validator import ClassValidator
...
class ClassAPI(BaseController):
    ...
    def create(self, **kw):
        ...
        try:
            ...
            # Validate data
            valid, message = ClassValidator.validate_creation_data(data)
            if not valid:
                return self.make_json_response({
                    "code": 400,
                    "status": "error",
    ...
    @http.route(['/api/classes/<id>'], type='http', auth="none", sitemap=False, cors='*', csrf=False, methods=['PUT'])
    ...
    @http.route(['/api/classes/copy'], type='http', auth="none", sitemap=False, cors='*', csrf=False, methods=['POST'])
    def mass_copy(self, **kw):
        _logger.info("API: Mass copy classes")
        try:
            dbname = self.get_db_name(kw)
            ...
            result = ClassController.mass_copy(dbname, id_list)
            return self.make_json_response(result)
        except json.JSONDecodeError:
            return self.make_json_response({
                "code": 400,
            ...
            return self.make_json_response(self.handle_exception(e, "mass copying classes"))
